...
Note that by inputing directly multiplications of operators, the output won't combine all the factors in the same mode to form a unified pattern item:
...
The multiplication of operators from the same mode can be taken into account by applying the `redo_mul` function (see `tools.redo_mul`):
